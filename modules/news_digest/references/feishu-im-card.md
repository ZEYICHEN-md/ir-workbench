...
2. **一、OTA/旅游行业新闻精选**：每条 `图标 + 加粗标题 + 2–3 句`，从定稿压缩，不另写判断
3. **三枚周度 KPI**：默认酒店 RevPAR / 航空客运量 / 机票票价；升绿降红，只作数字着色，块背景保持 `grey-50`
4. **二、行业数据更新**：国内酒店 / 国内航空 / 出境航空；周度 + 最近完整月度；脚注写清两个截至日
5. **打开行业数据看板** → https://datamax.fun

Header：`周报 | {月}月第{周}周`，副标题同时写情报主周和数据截至日。
...
| `FOOTNOTE` | 新闻期次与发布日、数据截至日、底稿文件名、来源、仅供内部参考 |

## 取数

- **新闻**：`outputs/news-digest/<期次键>/旅行行业新闻精选-*.md` 已定稿。标题沿用图标语义；正文可缩短，但数字和 so-what 不得改口径。不写携程当事方。
