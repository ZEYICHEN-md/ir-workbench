...
专家访谈相对新闻和公开财报的主要价值，是提供公开渠道难以获得的行业事实、公司信息、内部经营数据和因果机制。Agent 每次读取访谈时，先逐项判断哪些信息相对现有情报库和公开口径有增量，并生成 `intel_entries` 草稿；这一步覆盖整批访谈，不受飞书 `include` 影响。

进入情报草稿的每条信息必须写清公司、主题、事实或表述、专家原话、PDF 文件名和页码/位置。所有条目强制 `channel=expert-call`、`sensitivity=internal`；不论 `kind` 是 action 还是 statement，都必须保留 `quote` 和 `quote_where`。摘要转述不能冒充原话。草稿经人工核对后，先走公司情报库 dry-run，明确确认后才正式入库。
