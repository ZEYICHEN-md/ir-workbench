...
`str-plan` 只读。将来写入时默认**只填空格**，已有值一律保留（同飞书投影的规则）。

原因是底稿 2026 年 1–6 月的月度值口径不统一：有的来自 STR 官方月报（与周聚合差
0.7–0.8pp，因为月报样本集更大）、有的来自券商、还出现过券商**预测值**。
所以历史月份报冲突是预期的，不是错误。要统一历史口径会改动看板上的历史曲线，属另一件事。
