...
**它是唯一会在一次受控提交中同时写 Airline Data 与指标底稿四个航空同比格的自动化，
但不是指标底稿的唯一自动写入路径。**`industry-data str-apply` 也会在明确确认后填酒店 STR 空格。
ADR 0001 定了 Excel 是唯一真源，人手填（酒店 STR、周度航空）与两条受控自动化写入在同一张表上汇合。
所以本管道门禁最严，且写入路径做了四重保护：
