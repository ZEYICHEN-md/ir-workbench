...
#: 建档层里**预期条目稀疏**的几家（2026-08-25 裁定）。
#:
#: 回填 4 期后这三家 0 条，`ir doctor` 报了 warn。裁定是：**名单不动，它们该建档**，
#: 但这三家确实不是每周都有值得关注的动作——中文侧只收「携程非当事方」的境内事件，
#: 可收范围本来就窄（旧仓 ADR-003）。所以 0 条是覆盖事实，不是缺陷。
#:
#: 记在这里而不是把检查删掉：删掉就再也发现不了「本该有却漏采」的情况。
#: 现在它降为提示，措辞也说明是预期的。什么时候该重新看这条？——若某一家在
#: **连续多期都有明显公开动作却仍然 0 条**，那就是采集漏了，不是它安静。
SPARSE_EXPECTED: frozenset[str] = frozenset({"MEITUAN", "TCEL", "FLIGGY"})
