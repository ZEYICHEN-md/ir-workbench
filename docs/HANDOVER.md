...
sequenceDiagram
    autonumber
    actor IR as 👤 IR 分析师 (你)
    participant AI as 🤖 Agent 对话助理
    participant Sys as ⚙️ 工作台内核 (Control Plane)
    participant Data as 📊 权威底稿与情报库
    participant Out as 🌐 飞书 / 看板网站

    Note over IR, Out: 场景：每周一/周五例行业务流

    %% 行业数据流
    IR->>AI: “更新国内行业数据”
    AI->>Sys: 检查 Excel 底稿变动并比对差异
    Sys->>Data: 逐格扫描周度/月度数据变动
    Sys-->>AI: 自动识别变动范围，生成中文洞察草稿
    AI-->>IR: 展示修改点与洞察草稿：“请您审阅，确认无误请回复‘写入上线’”
    IR->>AI: “确认，写入上线”
    AI->>Sys: 触发上线门禁校验
    Sys->>Data: 刷新数据快照并自动生成英文洞察
    Sys->>Out: 发布并推送到 datamax.fun 看板
    Sys->>Sys: 自动回读验证线上内容
    AI-->>IR: ✅ 汇报：“已成功上线，看板已刷新！”

    %% 新闻精选流
    IR->>AI: “做这周的新闻精选”
    AI->>Sys: 自动召回资讯 + 跨期自动查重
    Sys-->>AI: 给出候选与未收录提示
    AI-->>IR: 呈现精选草稿（人把关/补充观点）
    IR->>AI: “正文确认了，发布吧”
    AI->>Sys: 自动导出 PDF/HTML、登入去重台账
    Sys->>Data: 自动沉淀入竞对情报库（按公司/主题打标）
    Sys->>Out: 写入飞书“旅业资讯库”与历史汇总
    Sys->>Out: 回读飞书文档校验条目数与附件
    AI-->>IR: ✅ 汇报：“新闻已发布至飞书，情报已入库！”
```
...
> - **季度财报跟踪**（`peers-appendix`）：季度末说 *“BKNG 出财报了，做业绩总结”*，AI 自动完成建表、贴图与格式检查。

---

## 四、 系统的四大安全机制：为什么你可以完全放心？
