...
> 九个域的完整开口与产出见 [CAPABILITIES.md](CAPABILITIES.md)。上图把它们收成一块，避免让人以为只有五条流水线。
...
下表是日常最常用的几条。Peers 财务模型、机构股东名册、港股查询、卖方研报摘读同样在工作台里，开口方式见 [CAPABILITIES.md](CAPABILITIES.md)。

| 模块名称 | 解决什么问题 | 你怎么对 AI 开口 | 产出物与去向 |
| :--- | :--- | :--- | :--- |
| **1. 旅行行业新闻精选与周报**<br>`news-digest` | 每周汇总 OTA 与旅游行业重大事件、竞对动态，提炼具体数字与含义，制作交付物。 | 🗣️ *“做 2026年8月第3周的新闻精选”*<br>🗣️ *“做这周的飞书卡片周报”* | ① **Markdown / HTML / PDF** 三份本地文件<br>② 你确认后才发布到**飞书 Wiki**<br>③ 定稿后自动打标存入**竞对情报库**<br>④ 另说「做飞书卡片周报」才生成**飞书群互动卡片** |
| **2. 国内行业数据与看板**<br>`industry-data` | 维护国内酒店 STR、周度航空等高频数据，自动发现变动、生成中英文洞察、同步线上可视化看板。 | 🗣️ *“更新国内行业数据”*<br>🗣️ *“用这份最新的中金周报更新看板”* | ① 自动比对数据差异<br>② 自动生成**中文/英文核心洞察**<br>③ 你说「写入上线」后才更新 **datamax.fun 看板** |
...
> **另外几条按需开口即可**：
> - **港股市场查询**（`hk-market`）：*“查一下港美成交占比现在多少”*
> - **卖方研报摘读**（`sellside-research`）：把 PDF 拖进对话框，*“帮我摘一下这份报告”*
> - **Peers 财务模型**（`peers-model`）：*“这是 BKNG 26Q3 财报，更新 Model”*（只出副本，不覆盖原文件）
> - **机构股东名册**（`shareholder-list`）：Capital IQ 底表放在「下载」里，*“更新 shareholder list，有效日切成今天”*
> - **季度口径检索**：先问情报库 *“Booking 这季度做了什么”*。Appendix 正文仍由人写。
...
3. **第三步：一键发布与多方沉淀**
   ...
   - AI 会自动：
     - 生成本地 PDF 与 HTML 文件；
     - **先问你是否发布到飞书 Wiki**，你说可以之后才写入部门知识库；
     - 把当期新闻要点存进竞对情报库；
   - 随后如果你需要往内部群发卡片，对 AI 说：👉 **「做这周的飞书卡片周报」**，AI 就会把做好的卡片私聊发到你的飞书上，你再转发到群即可。
...
1. **拖进对话框**（中金周报、STR、研报、财报 PDF、专家访谈）。
2. **留在电脑「下载」里**然后说一声（股东名册的 Capital IQ 底表就是这样）。
3. **新的行业数据 Excel** 放到 `data/workbooks/`，再说「换一份新的国内行业数据」。

要取回成品，直接问：「这期新闻精选在哪」「BKNG 模型副本在哪」「股东名册在哪」。

如果你想自己点开文件夹：

```text
IR_workbench/
├── data/workbooks/     # 改数只改这里被锁定的那份《国内行业数据》
├── outputs/            # 成品：新闻精选、模型副本、股东名册
├── inputs/             # 当期原件（Agent 会代为放好）
└── scratch/            # 临时草稿，可以清，不要当档案
```

过期临时文件对 AI 说「清理过期的临时文件」；它会先列出，你确认后才删。底稿归档和情报库不会被清掉。
