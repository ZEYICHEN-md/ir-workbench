...
只在第一次拿到工作台、或换了新电脑时做。做完之后，日常只开口说话。

### 第 1 步：装软件和两个搜索插件

| 要装的 | 干什么 | 怎么装 |
|---|---|---|
| Cursor | 你和系统说话的窗口，公司有会员 | 官网安装，登录公司账号 |
| Python 3.14 或更高 | 工作台运行环境 | 安装时勾选 Add to PATH；搞不定找交付人或 IT |
| Node.js（LTS） | 装飞书工具的前提 | 官网安装，一路下一步 |
...
1. 从交付人处拿到完整的 IR_workbench 文件夹（GitHub 私有仓 ir-workbench，里面已有行业数据底稿和三份 Peers Model），放到自己电脑上。
2. 用 Cursor 打开这个文件夹。
