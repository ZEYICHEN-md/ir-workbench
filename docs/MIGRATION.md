...
### 🔄 第 3.5 步：`expert-calls`（2026-09-01 代码迁入；真实双分支已运行，情报处置仍有 6 条待对账）
...
- [ ] **真实业务验收（情报分支）**：同批 34 条草稿中，A 类 11 条已全部入正式库；批准 deferred 文件的 B 类 8 条已全部进入 `deferred.jsonl`。但人工批准汇总写有 14 条暂缓，原编号 5、6、11、18、19、25 共 6 条在正式库、待核池和剔除清单中都没有最终处置记录。详见 `docs/specs/2026-09-03-expert-calls-acceptance-audit.md`

现有真实批次已证明两条分支均实际运行，不必机械等待下一批。当前保持 `partial`，是因为本批 6 条处置差异尚未由用户确认并逐 ID 对账；不能把飞书 manifest 的 6/6 或部分入库包装成情报分支整批完成。
